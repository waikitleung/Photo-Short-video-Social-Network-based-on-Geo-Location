...
const (
	INDEX    = "around" //ES could provide multiple service for different users the index indicate which users requests service
	TYPE     = "post"
	DISTANCE = "200km"
	// Needs to update
	...
	ES_URL      = "http://35.239.143.146:9200"
...
func handlerPost(w http.ResponseWriter, r *http.Request) {
	...
	im, header, _ := r.FormFile("image") // 在这里需要在读一次 因为上一次的saveToGCS 之后buffer为空
	defer im.Close()
...
func handlerSearch(w http.ResponseWriter, r *http.Request) {
	...
	lat, _ := strconv.ParseFloat(r.URL.Query().Get("lat"), 64) // we get two return value
	// the second one set _ because we don't care the value and we CAN'T define a var without using it
